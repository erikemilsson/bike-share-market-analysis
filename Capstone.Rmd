...
I can now remove the other columns as they won't be used
```{r}
...
```{r}
  ggplot(data=df_nocoordinates3) + geom_bar(mapping = aes(x=Rider_Type))
```
There are more annaual than casual riders per ride.
...
```
We see that that there are no docked bikes used at all by annaul members.  We also see that the percentage of classic bikes used by members is much greater than it is for casual riders. Casual riders seem to prefer electric bikes over other types of bikes.
