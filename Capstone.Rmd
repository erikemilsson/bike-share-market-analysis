...
Now add another column that shows the total time the riders used the bikes for, buy taking the difference between 'started_at' and 'ended_at' and calling the new column header 'Total_Time'. I can also remove the columns that won't be used at the same time:
...
```{r}
boxplot(df_nocoordinates3$Total_Time)
```

```{r}
max(df_nocoordinates3$Total_Time)
```

```{r}
ggplot(data = df_nocoordinates3, mapping = aes(x = Total_Time)) +
  geom_histogram()
```
