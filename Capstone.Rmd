...
The dataset that will be used to answer the business task is Cycilstic's historical trip data for the previous 12 months. The dataset was downloaded in a zip file with a folder for each month, each containing data in CSV-format. The data ranges from october 2021 to September 2022, using ISO date notation (YYYYMMDD). The data is imported into R and then append into one dataframe.
...
head(df)
```
The data from the CSV files has 13 columns from the start, which contain information such as unique rider IDs, bike type, start & end times, locations and type of rider. At first glance the headers need to be changed to be more descriptive and there are missing values in the station names and IDs that need to be considered.
...
The skim_without_charts function doesn't show the unique values of coordinates, so this is instead shown in the following graph:


``{r Count of unique values of geographical coordinates, message=FALSE}
df_1 %>%
...
* Longitudes and latitudes have about 20000-60000 unique values, many more compared to 1000-2000 for start and end station names. 

Due to the discrepancy in unique values between the coordinates and the station names, it will not be possible to do a one-to-one match the two. *By looking at the missing data it should give some ideas as to which columns can be dropped for graphing location data.*
...
df_3$Trip_Time_Minutes <- as.numeric(df_3$Trip_Time_Minutes) #change format of trip time column to numeric 

df_3$Trip_Time_Minutes <- round(df_3$Trip_Time_Minutes, 0) #round to nearest minute
```

Now to look at the Trip Time more closely:
```{r Trip time, eval=FALSE, include=FALSE}
ggplot(data = df_3) +
  geom_point(mapping = aes(x = Trip_Time_Minutes, y = Bike_Type)) +
  facet_wrap(~Rider_Type) +
  labs(title="Total time per ride", x = "Minutes", y = "")
```
Docked bikes seem to include times that are much greater (0-40,000minutes) than electric and classic bikes (0-2,000minutes) in travel time. This should be investigated further. 

There also appears to be some datapoints that are less than 0 for members using electric bikes. 
```{r}
paste(sum(df_3$Trip_Time_Minutes < 0),"rows with trip times below 0 minutes have been removed.")

df_clean <- df_3[df_3$Trip_Time_Minutes > 0, ]

rm(df,df_1,df_2,df_3) #remove other dataframes to save memory
```
