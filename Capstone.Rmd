...
information https://ride.divvybikes.com/data-license-agreement. The past 12 months 
(October 2021 to September 2022) of historical trip data was downloaded from the 
database in CSV format to a Google Drive location, and thereafter uploaded to 
BigQuery. There the 12 files were appended and thereafter exported as a CSV file 
to be read into this R Markdown report. 

The geographical coordinates columns (longitudes and latitudes for start and end 
stations) were removed as this analysis will not be using them. 
...
```{r Information about missing and unique values, echo=FALSE, message=FALSE}
df_skim %>%
...
From the skim function output the following information about *unique* observations 
can be seen:

* Each observation (total of 5,828,235) have unique Ride_IDs.
...
* The unique values of all the locations are much greater than the "over 600 
  ...
  Thus, choosing to omit either columns for name or ID should not have any effect on the missing data. I will 
