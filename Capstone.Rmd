...
head(df)
```
The headers are made of 13 columns, which contain information such as unique rider IDs, bike type, start & end times, locations and type of rider. At first glance the headers need to be changed to be more descriptive and there are some missing values in the station names and IDs.
...
The skim_without_charts function doesn't show the unique values of coordintes, so this is done here:

```{r}
#length counts the unique strings output
paste("There are",length(unique(df_renamed$start_lat)),"and", length(unique(df_renamed$start_lng)), "unique start station latitudes and longitudes.")
paste("There are",length(unique(df_renamed$end_lat)),"and", length(unique(df_renamed$end_lng)), "unique start station latitudes and longitudes.")
```
```{r}
df_renamed %>%
  group_by(Bike_Type, Rider_Type) %>%
  summarise("Unique start station latitudes" = n_distinct(start_lat), "Unique start station longitudes" = n_distinct(start_lng), "Unique end station latitudes" = n_distinct(end_lat), "Unique end station longitudes" = n_distinct(end_lng)) %>% 
  mutate(Xaxis = str(Bike_Type), .before = 1)
...
#percentage of riders using docked bikes
df_nocoordinates3 %>%
  group_by("Rider Type" = Rider_Type,"Bike Type" = Bike_Type) %>%
  summarise(mean = mean(Trip_Time_Minutes), n= n()) %>% 
  mutate("Ride Time in Years" = mean*n/60/24/365) %>% 
  ggplot() +
  geom_bar(mapping = aes(x = Rider_Type)) +
  facet_wrap(~Bike_Type)
```
...
Next I want to look specifically the riding behavior of the riders of docked bikes that ride for a "long" time, arbitrarily chosen to be over 2,000minutes or about 30 minutes, which is close to the maximum trip time for any other bike.
...
df_nocoordinates3_dockedhigh %>% 
  ...
  ggplot(aes(start_station_id)) + geom_bar(aes(fill=start_station_id))
...
```{r}
ggplot(data = df_nocoordinates3, aes(x = Trip_Time_Minutes, y = Round_Trip)) +
  facet_wrap(~Bike_Type) +
  geom_point() +
  labs(title="Total time per ride", x = "Minutes", y = "")
```
...
ggplot(data = df_nocoordinates3, aes(Trip_Time_Minutes, after_stat(count), fill = Round_Trip)) +
...
* See why some docked bike rides are up to 40000minutes long (almost 10 days). These riders are likely more intersted in a membership because they're paying a lot for the rentals by only being casual riders.
