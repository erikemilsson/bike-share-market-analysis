...
At first glance the headers need to be changed to be more descriptive and there are some missing values in the station names and IDs.
...
### Missing values
There are missing observations for some of the columns in the head. Using the skimr function gives more insight into how often they occur and for what columns.
```{r}
skim_without_charts(df_renamed)
```
Unique observations:
* There are over 5,828,235 observations (rows) and the exact same number of unique Rider_IDs, which likely means that the same person will be given a new Rider_ID every time they ride the bike. Renaming Rider_ID to Trip_ID would then make more sense given that this is the case, although in a non-fictional scenario it would be best to double-check this with the Cyclistic marketing analyst team that collect the data.
* There are 3 unique values (n_unique) for Bike_Type and 2 unique values for Rider_Type.
```{r}
df_renamed2 <- df_renamed1 %>% 
  rename(Trip_ID = Rider_ID)
```
...
Due to the missing values for the geographical data, the data has to be cleaned and since the changes 


. Since locations of the stations should correspond to their respective coordinates, I'll see if there is a possibility to to fill the missing stations with the corresponding station name obtained from matching station-coordinate pairs. 
...
unique_start_station_name <- length(unique(df_renamed2$start_station_name))
unique_start_station_id <-length(unique(df_renamed2$start_station_id))
unique_start_station_lat <- length(unique(df_renamed2$start_lat))
unique_start_station_lng <- length(unique(df_renamed2$start_lng))
unique_end_station_name <- length(unique(df_renamed2$end_station_name))
unique_end_station_id <- length(unique(df_renamed2$end_station_id))
unique_end_station_lat <- length(unique(df_renamed2$end_lat))
unique_end_station_lng <- length(unique(df_renamed2$end_lng))

paste("There are",unique_start_station_name, "unique start station names and", unique_end_station_name, "unique end station names")

paste("There are",unique_start_station_id, "unique start station ids and",unique_end_station_id,"unique end station ids")

paste("There are",unique_start_station_lat, "unique start station latitudes and",unique_end_station_lat,"unique end station latitudes")

paste("There are",unique_start_station_lng, "unique end station longitudes and",unique_end_station_lng,"unique start station longitudes")
```
...
```{r}
df_nocoordinates3 <- round(df_nocoordinates2$Total_Time_Minutes, 0)
```
...
```{r}
df_nocoordinates4 <- df_nocoordinates3 %>%
