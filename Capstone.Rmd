...
Now to look at missing values and unique values from the cleaned dataset.
...
looking at the results so as to not make conculsions and decisions on this 
misleading data. 
...
ggplot() +
  ...
  labs(title="Annual count of rides per bike and rider type", x = "", y = "Rides [count]")
```
...
df_clean %>%
  ...
  labs(title="Annual Mean ride times per bike and rider type", x = "", y = "duration [minutes]")
```
...
ggplot(df_clean,
  ...
  labs(title = "Distribution of trip times under 1 hour", 
       x = "Ride count distribution", y = "Rides [count]")
```
